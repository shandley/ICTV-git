...
def create_species_growth_trajectory(data: Dict) -> None:
    ...
    line = ax.plot(years, species_counts, 'o-', linewidth=4, markersize=10, 
                   color='#2E86AB', label='Total Viral Species', markeredgecolor='white', markeredgewidth=2)
    
    # Add exponential trend line
    # Fit exponential curve to real data
...
Data source: {data["data_source"]}'''
    ...
    ax.text(0.02, 0.98, real_stats, transform=ax.transAxes, fontsize=12,
            verticalalignment='top', bbox=props, family='monospace')
    
    # Formatting
    ax.grid(True, alpha=0.4)
    ax.legend(fontsize=13, loc='center right', frameon=True, fancybox=True, shadow=True)
